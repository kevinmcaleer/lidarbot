...
# Setup timed refresh (every 0.5 seconds)
clear_radar()
timer = Timer()
timer.init(period=100, mode=Timer.PERIODIC, callback=print_radar)

print("Streaming points...")
        ...
        if second == b'\x2c':
            ...
            while points_read < points_expected:
                ...
                if len(point_buffer) >= 3:
                    ...
                    if confidence > 50:
                        # instead of interpolating, just use fixed step!
                        interp_angle = (start_angle + points_read * 0.5) % 360
